...
canonical CSVs beneath ``Data/Additional Sensitivity Analysis``.
"""
...
SOURCE_ROOT = REPOSITORY_ROOT / "Data/Provenance/00_execution_manifest/source_scenes/active_r2"
REGISTRY = REPOSITORY_ROOT / "Data/Design/01_multi_aoi/final_four_aoi_registry.geojson"
FROZEN_PRIMARY_PAIRS = REPOSITORY_ROOT / "Data/Inputs/paired_observations.csv.gz"
...
def assert_output_root(path: Path) -> Path:
    ...
    frozen = [(REPOSITORY_ROOT / "Data/Inputs").resolve(), (REPOSITORY_ROOT / "Data/Results").resolve(),
              (REPOSITORY_ROOT / "Data/Provenance").resolve(), (REPOSITORY_ROOT / "Data/Legacy_AOI00").resolve()]
...
def materialize(sensor: str, *, sensitivity: str, variant: str, output_csv: Path,
    ...
    for aoi, feature in sorted(features.items()):
        ...
        for year in range(2021, 2026):
            ...
            group = pd.DataFrame(group_rows); assert_pair_schema(group.columns)
            ...
            checkpoint_manifest.write_text(json.dumps({"aoi": aoi, "year": year, "sensor": sensor, "sensitivity": sensitivity,
                "variant": variant, "rows": len(group), "output_sha256": hashlib.sha256(checkpoint.read_bytes()).hexdigest(), "status": "COMPLETED"}, indent=2) + "\n", encoding="utf-8")
            all_rows.extend(group_rows)
    frame = pd.DataFrame(all_rows); assert_pair_schema(frame.columns)
